...
export function Inventory() {
  ...
  const lastLookupStatusRef = useRef<"unused" | "searching" | "found" | "not_found">("unused");
  ...
  useEffect(() => {
    fetchProducts("init");
    // fetchProducts is a closure over component state used elsewhere (next/prev buttons);
    // mount-only fetch is the intended behavior — deps array stays empty.
  ...
  const handleBarcodeLookup = async (barcode: string) => {
    ...
    lastLookupStatusRef.current = "searching";
    try {
      ...
      if (data.name) {
        ...
        lastLookupStatusRef.current = "found";
      } else {
        lastLookupStatusRef.current = "not_found";
      }
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const cleanBarcodesList = (formData.barcodes || []).reduce<string[]>((acc, bc) => {
      const cleaned = bc.trim().replace(/[^a-zA-Z0-9]/g, "");
    ...
    <div className="flex flex-col space-y-8 max-w-[1600px] mx-auto p-4 md:p-8">
        ...
        <div className="flex flex-wrap gap-3">
          {isAdmin && (
            <button type="button"
              ...
              className="bg-white border border-slate-200 text-slate-700 font-bold px-6 py-3 rounded-2xl shadow-sm hover:bg-slate-50 hover:-translate-y-0.5 transition-all flex items-center gap-x-2"
            >
          ...
          <button type="button" 
            onClick={() => openModal()}
            className="bg-indigo-600 text-white font-bold px-6 py-3 rounded-2xl shadow-xl shadow-indigo-200 hover:bg-indigo-500 hover:-translate-y-0.5 transition-all flex items-center gap-x-2"
          >
            <Plus size={20} />
          ...
          <div className="flex overflow-x-auto gap-4 pb-2 scrollbar-hide">
            {smartActions.map((action, idx) => (
              <motion.div
                ...
                className={cn(
                  "min-w-[300px] p-5 rounded-[2rem] border flex items-start gap-x-4 shadow-sm",
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="bg-white rounded-[2.5rem] p-8 border border-slate-200 flex items-center justify-between overflow-hidden relative group shadow-sm">
          ...
          <div className="size-16 bg-emerald-50 rounded-[1.5rem] flex items-center justify-center rotate-12 group-hover:rotate-0 transition-transform">
      ...
      <div className="bg-white p-4 rounded-3xl border border-slate-200 flex flex-col lg:flex-row gap-6 items-center shadow-sm">
        <div className="flex bg-slate-100 p-1 rounded-2xl w-full lg:w-auto overflow-x-auto scrollbar-none whitespace-nowrap">
          ...
          <button type="button"
            ...
            className={cn(
              "flex-1 lg:flex-none px-3 sm:px-6 py-2.5 text-[10px] sm:text-xs font-bold uppercase tracking-wider rounded-xl transition-all flex items-center justify-center gap-x-1 sm:gap-x-2 cursor-pointer",
        ...
        <div className="relative flex-1 w-full flex gap-3">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              aria-label="Filtrar inventario por nombre, SKU o categoría"
            ...
            <button type="button" 
              ...
              className="absolute right-3 top-1/2 -translate-y-1/2 size-8 bg-white rounded-lg flex items-center justify-center text-slate-400 hover:text-indigo-600 transition-colors shadow-sm"
                ...
                >
                  <td className="px-8 py-5">
                    <div className="flex items-center gap-x-4">
                        <div className="size-12 bg-white border border-slate-100 rounded-2xl flex items-center justify-center text-slate-400 shadow-sm overflow-hidden">
                          {product.image ? (
                  ...
                  {isAdmin && (
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-x-1 font-black text-slate-700">
                  ...
                  <td className="px-8 py-5">
                    {Number(product.stock) <= 0 ? (
                      <span className="inline-flex items-center gap-x-1.5 px-3 py-1 bg-rose-50 text-rose-600 text-[10px] font-black uppercase tracking-widest rounded-lg border border-rose-100">
                        <span>Agotado</span>
                      </span>
                    ) : (
                      <span className="inline-flex items-center gap-x-1.5 px-3 py-1 bg-emerald-50 text-emerald-600 text-[10px] font-black uppercase tracking-widest rounded-lg border border-emerald-100">
                        <span>En Stock</span>
                      </span>
                    )}
                  </td>
                  <td className="px-8 py-5 text-right">
                    <div className="flex items-center justify-end gap-x-2">
                      <button type="button" onClick={() => openModal(product)} className="p-2 text-slate-400 hover:text-indigo-600 transition-colors">
                        <Edit2 size={18} />
                      </button>
                    </div>
                  </td>
          ...
          {filteredProducts.map(product => (
            <div key={product.id} className="p-6 flex flex-col space-y-4">
               <div className="flex items-center justify-between">
                  ...
                  <button type="button" onClick={() => openModal(product)} className="size-10 bg-slate-50 rounded-xl flex items-center justify-center text-slate-400">
                    <Edit2 size={18} />
                  </button>
               </div>
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 md:p-8 flex flex-col space-y-6 text-slate-700">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div className="sm:col-span-2">
                    ...
                    <div className="flex gap-4">
                      ...
                      />
                      {formData.image && (
                        <div className="size-14 bg-slate-100 rounded-2xl border border-slate-200 overflow-hidden shrink-0">
                          <img src={formData.image} alt={formData.name ? `Vista previa de ${formData.name}` : "Vista previa del producto"} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                        </div>
                      )}
                    </div>
                  </div>
                  <div className="sm:col-span-2 flex flex-col space-y-4">
                    ...
                    <div className="flex flex-wrap gap-2 mb-2">
                      {formData.barcodes.map((bc) => (
                        <span key={bc} className="inline-flex items-center gap-x-2 bg-indigo-50 text-indigo-600 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-tight">
                          <span>{bc}</span>
                          <button
                            type="button"
                            aria-label={`Eliminar código ${bc}`}
                            onClick={() => setFormData({...formData, barcodes: formData.barcodes.filter(b => b !== bc)})}
                  ...
                  <div className="sm:col-span-2">
                    <label htmlFor={inputId("image")} className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Imagen URL (Opcional)</label>
                    <input
                      id={inputId("image")}
                      type="url"
                  ...
                  <div>
                    <label htmlFor={inputId("sku")} className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">SKU / ID Interno</label>
                    <input
                      id={inputId("sku")}
                      type="text"
                  ...
                  <div>
                    <label htmlFor={inputId("costPrice")} className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Costo Unitario</label>
                    <div className="relative">
                      <DollarSign className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={16} />
                      <input
                        id={inputId("costPrice")}
                        required
                        type="number" step="1"
                  ...
                  <div>
                    <label htmlFor={inputId("price")} className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Precio Venta</label>
                    <div className="relative">
                      <DollarSign className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={16} />
                      <input
                        id={inputId("price")}
                  ...
                  <div>
                    <label htmlFor={inputId("wholesalePrice")} className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Precio Mayorista (Default: Venta)</label>
                    <div className="relative">
                      <DollarSign className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={16} />
                      <input
                        id={inputId("wholesalePrice")}
                        required
                        type="number" step="1"
                        className="w-full h-14 bg-slate-50 border border-slate-100 rounded-2xl pl-10 pr-5 text-sm font-bold focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:bg-white transition-all text-slate-800"
                  ...
                  <div>
                    <label htmlFor={inputId("wholesaleMinQty")} className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Mínimo para Mayorista (Unidades)</label>
                    <div className="relative">
                      <Package className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={16} />
                      <input
                        id={inputId("wholesaleMinQty")}
                        required
                        type="number" min="1"
                        className="w-full h-14 bg-slate-50 border border-slate-100 rounded-2xl pl-10 pr-5 text-sm font-bold focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:bg-white transition-all text-slate-800"
                  ...
                  <div className="sm:col-span-2">
                    <label htmlFor={inputId("categoryId")} className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Categoría</label>
                  ...
                  <div className="sm:col-span-2">
                    <label htmlFor={inputId("supplierId")} className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Proveedor</label>
                    <select
                      id={inputId("supplierId")}
                      className="w-full h-14 bg-slate-50 border border-slate-100 rounded-2xl px-5 text-sm font-bold focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:bg-white transition-all text-slate-800 appearance-none"
                  ...
                  <div>
                    <label htmlFor={inputId("stock")} className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Stock Actual</label>
                    <input
                      id={inputId("stock")}
                      required
                      type="number"
                  ...
                  <div>
                    <label htmlFor={inputId("minThreshold")} className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-2 block">Min. Emergencia</label>
                    <input
                      id={inputId("minThreshold")}
                      required
                      type="number"
                      className="w-full h-14 bg-slate-50 border border-slate-100 rounded-2xl px-5 text-sm font-bold focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:bg-white transition-all text-slate-800"
                ...
                <div className="pt-8 border-slate-50 flex flex-col sm:flex-row gap-3 w-full">
                  ...
                  <button 
                    type="submit"
                    className="w-full sm:flex-[2] py-4 bg-indigo-600 text-white font-bold uppercase tracking-wider text-[13px] rounded-2xl hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-100 flex items-center justify-center gap-x-2 min-h-[3.25rem] md:min-h-[3.5rem] cursor-pointer"
                  >
                    <Save size={18} />
