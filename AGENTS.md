# StockFlow — Codex Operating Manual
...
### Premium stockflow-specific (`.Codex/agents/*.md`) — invoke via `@agent-<name>`
...
### DEE generic kept (`.Codex/agents/*_agent.md`) — useful for stockflow
...
10. **Skipping audit on configs in a merge**: when fusionando work from another agent, ALWAYS `git show --stat <commit>` to see ALL files changed, not just the visible refactor. Config files (`.gitignore`, `package.json`, lockfiles, `.Codex/`) are where silent regressions hide.

## 12. MCPs configured (`.Codex/settings.json`)

10 MCPs, all without API keys by default. Activate with `bash .Codex/setup-wizard.sh .`.
...
1. **Before any non-trivial change**: read this AGENTS.md, then read the specific files referenced in §3.
...
4. **Before merging from another agent's branch**:
   ...
   - Audit `.gitignore`, `package.json`, lockfiles, `.Codex/`, `functions/.gitignore` for silent deletions.
...
- **`d29a457`** (2026-05-23): silently deleted `firebase-admin` from `package.json`, removed `serviceAccountKey.json` from `.gitignore`, deleted `pnpm-lock.yaml`, deleted `functions/.gitignore`, deleted 8 premium agents from `.Codex/agents/`. Resolved in `9e3a197` + `eb3abea`. **Lesson encoded in §11 anti-pattern 10**.
...
## 15. Available Skills (`.agents/skills/`)

Skills viven en `.agents/skills/<name>/SKILL.md` y son **auto-descubiertos** por Antigravity al iniciar la sesión (no requieren registro en config). Cada `SKILL.md` tiene frontmatter `name + description` que el agente lee para decidir si aplica. Instalación / actualización vía `npx skills add <repo>` (paquete `skills` de npm) — el último install activo proviene de `Leonxlnx/taste-skill` (2026-05-27).

**Antes de invocar uno**: leer `.agents/skills/<name>/SKILL.md`. El installer advierte explícitamente: *"they run with full agent permissions"*. No invocar a ciegas.

### 15.1 Frontend / UX — territorio Antigravity

| Skill | Cuándo invocar |
|---|---|
| `design-taste-frontend` | Landing pages, portfolios, hero sections, copy editorial. Anti-slop premium. **Skill principal de taste-skill.** |
| `design-taste-frontend-v1` | Solo si una pantalla depende del comportamiento exacto del v1 (backward compat). El default es v2. |
| `high-end-visual-design` | Cualquier pantalla pública que necesite "feel premium" (Sf Client portal, landing del shop). |
| `minimalist-ui` | Pantallas editorial limpias, paleta warm monochrome. NO usar para dashboards. |
| `mobile-design` | Diseñar / rediseñar pensando mobile-first (Sf Client, MobilePOS, DriverPWA). |
| `redesign-existing-projects` | Refactor visual de pantalla existente sin romper funcionalidad. Audit-first. |
| `industrial-brutalist-ui` | Dashboards densos de datos / portfolios brutalistas. Útil para pantallas internas de Logistics/Inventory si Pierre lo aprueba. |
| `gpt-taste` | GSAP motion + scroll engineering. Solo para landings con scroll storytelling. |
| `stitch-design-taste` | Genera `DESIGN.md` semánticos para Google Stitch. Pre-step para concepts. |
| `brandkit` | Brand boards, logo systems, identity decks. Trabajo de identidad visual. |
| `imagegen-frontend-mobile` | Mockups de pantallas mobile con frame iPhone (genera imágenes, no código). |
| `imagegen-frontend-web` | Mockups de landings web — **1 imagen por sección**, no comprimir. |
| `image-to-code` | Generar imagen → analizar → implementar. Para tareas visuales importantes. |

### 15.2 Dev tools — cualquier agente

| Skill | Cuándo invocar |
|---|---|
| `react-doctor` | Pre-commit, post-feature, score regression check. Lint + a11y + bundle. |
| `code-review` | Antes de pushear cambios grandes. Modo `low/medium/high/ultra`. |
| `simplify` | `code-review --fix` aplicado directo al working tree. |
| `security-review` | Después de tocar `firestore.rules` / payments / auth. **Solo CC-auditor**. |
| `verify` | Confirmar que un fix funciona en runtime real (no solo tests). |
| `run` | Lanzar la app para ver un cambio funcionando. |
| `init` | Crear/regenerar CLAUDE.md / AGENTS.md desde el código. |
| `review` | Review un pull request. |
| `full-output-enforcement` | Forzar output completo sin truncar — útil para código largo. |

### 15.3 Restricciones — Antigravity NO debe invocar

- `security-review` — es trabajo de CC-auditor (Claude Code), no de UX/UI.
- `update-config`, `fewer-permission-prompts` — modifican `settings.json` del harness; decisión de Pierre.
- `claude-api` — es para apps que importan `@anthropic-ai/sdk`, no aplica al stack StockFlow.
- Cualquier skill que toque archivos en tier-blacklist (ver `.agents/blacklist` si existe, o regla durable en [collaboration_rules](memory)).

### 15.4 Cómo se actualizan / desinstalan

```bash
npx skills list             # ver instalados
npx skills add <repo>       # instalar
npx skills remove <name>    # desinstalar
```

Los skills viven en `.agents/skills/` (universal) con symlinks a `.claude/skills/` (para Claude Code) y rutas equivalentes para Qwen Code. **Está gitignored** — cada miembro del equipo corre el install command localmente.
