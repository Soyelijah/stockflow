...
export const setUserRole = onCall(async (request) => {
  ...
  if (!targetUid || typeof targetUid !== "string") {
    throw new HttpsError("invalid-argument", "Missing or invalid 'userId' or 'uid' parameter.");
  }
  ...
  try {
    ...
    // 3. Audit log — track every role change for compliance
    ...
    return { success: true, message: `Rol ${role} asignado al usuario ${targetUid}.` };
  } catch (error: any) {
